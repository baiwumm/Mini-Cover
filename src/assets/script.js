...
export const { canvas: bgCanvas, ctx: bgCtx } = createCanvas(1920, 1080);
export const { canvas: textCanvas, ctx: textCtx } = createCanvas(1920, 1080);
export const { canvas: watermarkCanvas, ctx: watermarkCtx } = createCanvas(1920, 1080);
export const { canvas: squareCanvas, ctx: squareCtx } = createCanvas(1920, 1080);
...
export function drawWatermark() {
  ...
  const { fontFamily } = getHtmlFontStyles();
  const font = state.selectedFont ? `${state.selectedFont}, ${fontFamily}` : fontFamily;
  watermarkCtx.font = `italic 30px ${font}`;
